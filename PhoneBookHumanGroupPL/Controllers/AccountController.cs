...
namespace PhoneBookHumanGroupPL.Controllers
{
    public class AccountController : Controller
    {
        ...
        public IActionResult Register(bool isClicked)
        {
            if (isClicked)
            {
                var pass = CreatRandomPass();
                ViewBag.Pass = pass;
            }
            return View();
        }
        ...
        public IActionResult ForgotPassword(string? email)
        {
            try
            {
                ...
                var randomPassword = CreatRandomPass(); 
        ...
        public string CreatRandomPass()
        {
            string chars = "0123456789ABCDEFGHIJKLMNOPQSTUVWXYZabcdefghijklmnpqrstuvwxyz?@!#%+$-_.*%";
            string PasswordRegex = @"^((?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])|(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[^a-zA-Z0-9])|(?=.*?[A-Z])(?=.*?[0-9])(?=.*?[^a-zA-Z0-9])|(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^a-zA-Z0-9])).{8,}$";
